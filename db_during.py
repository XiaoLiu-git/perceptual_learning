...
def ff_train(show_epc, net_name, slow_learning=True, bg_epoch=0,
    ...
    for epoch in range(200):  # loop over the dataset multiple times
        ...
        if epoch > bg_epoch:
            ...
            ## TPE_during training
            weight[5:15, :] = weight[5:15, :] + slow_lr * np.ones([10, 18])
...
for begin_epoch in [5, 10, 20, 30, 40, 50]:
    ...
    str_sl_lr = str(sl_lr).replace('.', '')
    name_head = foldername + '/db_dr' + str(begin_epoch) + '_' + str_sl_lr + net_name
