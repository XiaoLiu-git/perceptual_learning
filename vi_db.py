...
from visual_tool import visualization
...
def ff_train(show_epc, net_name, slow_learning=True, bg_epoch=0, slow_lr = 0.05):
    # input
    num_batch = 16
    ...
    for epoch in range(200):  # loop over the dataset multiple times
        ...
        inputs = np.reshape(ob.normalize(np.reshape(inputs, [num_batch, -1])),
                            size_input)
        if epoch > bg_epoch:
            weight[25:35, :] = weight[25:35, :] + slow_lr * np.ones([10, 18])
        ...
        if epoch == 50:
            _, feature_in, feature_out=visualization(net, 'readout')
            pdb.set_trace()
...
for net_name in net_list:
    for begin_epoch in [0, 5, 10, 20, 30, 40, 50]:
        for s in range(num_section):
            ...
            for db_i in range(200):
                ...
                for i in range(40 // loc):
                    ...
                    # plt.subplot(2, 4, i+1)
                    # plt.imshow(y[1].squeeze())
                    Acc_test_db[s,db_i, i, :] = ob.test(net, y, num_test * 2, t_label,
                                                print_test)[:-1]
                    t_data = np.roll(t_data, -5, axis=2)

                weight[5:15, :] = weight[5:15, :] + sl_lr * np.ones([10, 18])

            print(Acc_test_db[s,db_i])

        str_sl_lr = '_'+str(sl_lr).replace('.', '')
        name_head = foldername + '/db_af_fix_'+str(begin_epoch)+str_sl_lr + net_name
