...
def train(show_epc):
    ...
    loss_list = np.zeros(200 // show_epc)
    num_test = 100
    while np.any(acc_list[0,:]<0.1) or np.any(acc_list[0,:]>0.8):
        ...
        for i in range(num_test // 2):
            ...
            t_inputs[1 * num_test + 2 * i + 1, :, :, :] = representation(img_tg[1])
            ...
            t_inputs[2 * num_test + 2 * i + 1, :, :, :] = representation(img_tg[1])
        _epc = 1
    ...
    for epoch in range(200):  # loop over the dataset multiple times
        ...
        if epoch % show_epc == show_epc - 1:  # print every show epoch
            ...
            acc_list[_epc, 0] = acc
            loss_list[_epc-1] = running_loss / show_epc
            print('[%d] loss: %.6f' %
            ...
            running_loss = 0.0
            acc_list[_epc, 1:] = test(net, t_inputs, num_test, t_labels)[:3]
            _epc += 1
    return acc_list, loss_list


Acc, Loss = train(show_epc=2)
plt.style.available
...
plt.title("Train with FF",fontsize=24)
plt.ylabel("Loss")
plt.xlabel("No. epoch")

plt.subplot(222)
plt.plot(np.arange(0,202,2), Acc*100, linewidth=4, label=["train","test","loc2","ori2"])
plt.title("Train with FF",fontsize=24)
plt.ylabel("Accuracy %")
plt.xlabel("No. epoch")
