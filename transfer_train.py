...
def train(show_epc, net_name):
    ...
    if net_name is "CC3":
        net = Net_cc()
    elif net_name is "CC5":
        net = Net_cc5()
    elif net_name is "CC7":
        net = Net_cc7()
    elif net_name is "FC3":
        net = Net_fc()
    elif net_name is "FC5":
        net = Net_fc5()
    else:
        net = Net_fc7()
...
for net_name in net_list:
    ...
    np.save('./0927/'+net_name + '_acc.npy', AccALL)
    np.save('./0927/'+net_name + '_loss.npy', LossALL)
    plt.style.available
    ...
    plt.plot(np.arange(0, 202, 2), Acc * 100, linewidth=4,
             label=["train", "test", "loc*", "ori*"])
    plt.title("Train with " + net_name, fontsize=24)
    plt.ylabel("Accuracy %")
    plt.xlabel("No. epoch")
    plt.legend(loc='upper left')
    plt.savefig('./0927/'+net_name +'.jpg')
