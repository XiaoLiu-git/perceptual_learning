...
def ff_train(show_epc, net_name, slow_learning=True):
    ...
    ## TPE before training
    weight[5:15, :] = 0.05 * np.ones([10, 18]) * 50
...
for net_name in net_list:
    ...
    np.save(foldername + '/db_bf005' + net_name + 'AccALL.npy', AccALL)
    np.save(foldername + '/db_bf005' + net_name + 'LossALL.npy', LossALL)
    np.save(foldername + '/db_bf005' + net_name + 'Acc_test.npy', Acc_test)
    np.save(foldername + '/db_bf005' + net_name + 'Acc_testfx.npy', Acc_test_fx)

 ###TODO!!!测试0.01、1、0.5,0.05的结果是否有差异 测试从100个再开始的结果